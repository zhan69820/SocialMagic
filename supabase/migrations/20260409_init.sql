...
-- -----------------------------------------------------------------------------
-- contents RLS — 通过 profile_id 归属链验证
-- -----------------------------------------------------------------------------

CREATE POLICY "contents_select_own" ON public.contents
  FOR SELECT USING (
    profile_id = (SELECT id FROM public.profiles WHERE anon_id = current_setting('request.jwt.claims', true)::json ->> 'anon_id')
  );
...
-- -----------------------------------------------------------------------------
-- social_posts RLS — 通过 profile_id 归属链验证
-- -----------------------------------------------------------------------------
